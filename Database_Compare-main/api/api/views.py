    ...
    if request.method == 'GET':
        ...
        try:
            if data.get('query') == "createTable":
                ...
                time2 = time.time_ns()
                return JsonResponse({"query": 'CREATE TABLE Orders ("Item Type" Varchar(250), "Order Date" Date, "Order ID" Int, "Units Sold" Float, "Unit Price" Float)', "time": utils_DB.TimeDifferenceWithConversionToMiliseconds(time1, time2)}, status=200)
...
class ClickHouse:
    ...
    def __init__(self):
        ...
        self.client = clickhouse_driver.Client(
            'database_compare-main_clickhouse-server_1')
...
class PostGres:
    def __init__(self):
        ...
        self.client = psycopg2.connect(
            "dbname='postgres' user='postgres' host='database_compare-main_postgres_1' password='postgres'")
        self.cursor = self.client.cursor()

    # create table structure
    def createTable(self):
        data = self.cursor.execute('CREATE TABLE IF NOT EXISTS Orders ("Item Type" Varchar(250), "Order Date" Date, "Order ID" Int, "Units Sold" Float, "Unit Price" Float)')
        self.client.commit()
        return data
...
class MsSql:
    ...
    def __init__(self):
        ...
        self.client = pymssql.connect(
            "database_compare-main_mssql_1", "SA", "Pass@word", "master")
        self.cursor = self.client.cursor()
